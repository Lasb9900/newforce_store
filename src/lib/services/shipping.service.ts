...
function withLabels(options: ShippingOption[]) {
  ...
  const fastest = [...options].sort(
    (a, b) => Number(a.estimated_days.split("-")[0]) - Number(b.estimated_days.split("-")[0])
  )[0]?.id;
...
function toInternalOptions(
  subtotalCents: number,
  upsRates: Awaited<ReturnType<typeof getUpsRates>>
): ShippingOption[] {
...
function isMockFallbackAllowed() {
  return process.env.ALLOW_MOCK_SHIPPING_FALLBACK === "true";
}

export async function getShippingOptions(
  input: ShippingInput
): Promise<{ options: ShippingOption[]; source: "UPS_REAL" | "UPS_MOCK" }> {
  ...
  try {
    ...
    if (!isMockFallbackAllowed()) {
      console.error("[SHIPPING] UPS rate lookup failed and mock fallback is disabled", error);
