...
function renderOrderConfirmationEmail(order: OrderRecord): { html: string; text: string } {
  ...
  const itemsHtml = (order.order_items ?? [])
    .map((item) => {
      ...
      const lineTotalLabel = isPointsItem
        ? `${Number(item.points_price_snapshot ?? 0) * qty} pts`
        : formatCurrency(lineTotalCents, order.currency ?? "USD");
  ...
  const html = `
    <div style="margin:0;padding:32px 16px;background:#eef2f7;font-family:Arial,Helvetica,sans-serif;color:#111827;">
      <div style="max-width:680px;margin:0 auto;background:#ffffff;border:1px solid #dbe3ee;border-radius:18px;overflow:hidden;box-shadow:0 10px 30px rgba(15,23,42,0.08);">
        ...
        <div style="padding:30px 28px 12px 28px;">
          <div style="font-size:30px;font-weight:800;color:#111827;letter-spacing:-0.6px;margin-bottom:10px;">
            🎉 Your order is confirmed
          </div>

          <div style="font-size:15px;line-height:1.8;color:#374151;margin-bottom:10px;">
            Hi ${escapeHtml(order.buyer_name?.trim() || "there")},<br />
            ${escapeHtml(intro)}
          </div>

          <div style="font-size:13px;line-height:1.7;color:#6b7280;margin-bottom:22px;">
            Thanks for shopping with Liquidation Plus.
          </div>
          ...
          <div style="text-align:center;margin:0 0 28px 0;">
            <a
              ...
              style="display:inline-block;background:#111827;color:#ffffff;text-decoration:none;font-size:14px;font-weight:700;padding:14px 26px;border-radius:10px;box-shadow:0 4px 12px rgba(17,24,39,0.18);"
            >
              View your order
            </a>
          </div>
            ...
            <tr>
              <td style="vertical-align:top;width:50%;padding-right:10px;">
                <div style="border:1px solid #e5e7eb;border-radius:14px;padding:16px 16px 14px 16px;height:100%;background:#ffffff;">
                  <div style="font-size:12px;color:#6b7280;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:10px;">Shipping</div>
          ...
          <div style="background:#f8fafc;border:1px solid #dbe3ee;border-radius:16px;padding:18px 20px;margin-bottom:20px;">
            ...
            <div style="font-size:14px;line-height:1.8;color:#4b5563;">
              We’ll notify you when your order ships. You can review your purchase details anytime from your account.
            </div>
          </div>
