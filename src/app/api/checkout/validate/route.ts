...
export async function POST(req: Request) {
  ...
  for (const item of parsed.data.items) {
    if (item.variantId) {
      ...
      checked.push({ ...item, valid: !!data?.active && data.stock >= item.qty && Number(data?.price_cents ?? 0) > 0, unitPriceCents: data?.price_cents });
    } else {
      const { data } = await sb.from("products").select("id,base_price_cents,base_stock,active").eq("id", item.productId!).single();
      checked.push({ ...item, valid: !!data?.active && data.base_stock >= item.qty && Number(data?.base_price_cents ?? 0) > 0, unitPriceCents: data?.base_price_cents });
    }
  }
