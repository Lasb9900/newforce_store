...
export default function CartPage() {
  ...
  function shippingValid() {
    return Object.values(shipping).every((v) => v.trim().length >= 2);
  }

  async function checkout() {
    if (!shippingValid()) {
      alert("Completa los datos de entrega antes de continuar");
      return;
    }
  ...
  return (
    <div className="rounded-xl border border-uiBorder bg-surface p-5 shadow-sm space-y-4">
      ...
      <p className="mb-2 text-sm text-mutedText">Revisa cantidades, completa entrega y valida stock antes de checkout.</p>
      ...
      <section className="rounded border border-uiBorder p-4 space-y-3">
        ...
        <div className="grid gap-2 md:grid-cols-2">
          ...
          <input className="rounded border border-uiBorder p-2" placeholder="Dirección" value={shipping.address_line_1} onChange={(e) => setShipping((s) => ({ ...s, address_line_1: e.target.value }))} />
          ...
          <input className="rounded border border-uiBorder p-2" placeholder="Estado" value={shipping.state} onChange={(e) => setShipping((s) => ({ ...s, state: e.target.value }))} />
          <input className="rounded border border-uiBorder p-2 md:col-span-2" placeholder="Código postal" value={shipping.postal_code} onChange={(e) => setShipping((s) => ({ ...s, postal_code: e.target.value }))} />
        </div>
      </section>
