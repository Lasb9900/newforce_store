...
export default async function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const cover = images[0]?.url ?? "https://placehold.co/800x600?text=Product";
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <div className="space-y-3">
        <div className="relative h-[420px] overflow-hidden rounded-xl border border-uiBorder bg-surface">
        ...
        <div className="grid grid-cols-2 gap-3">
          {images.slice(0, 4).map((image: { id: string; url: string }, index: number) => (
            <div key={image.id ?? index} className="relative h-28 overflow-hidden rounded-lg border border-uiBorder bg-surface">
      ...
      <div className="space-y-4 rounded-xl border border-uiBorder bg-surface p-6 shadow-sm">
        ...
        <p className="text-mutedText">{product.description}</p>
