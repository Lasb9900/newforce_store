...
export default async function AccountPointsPage() {
  ...
  const [
    ...
    supabase
      .from("products")
      .select("id,name,points_price,base_stock")
      .eq("active", true)
      .eq("redeemable", true)
      .gt("base_stock", 0)
      .order("name"),
  ...
  const effectivePoints = Number(customerPointsRow?.balance ?? 0);
          ...
          {(ledger ?? []).map((entry) => (
            <li key={entry.id} className="rounded-md border border-uiBorder px-3 py-2">
              <p className="font-semibold">
                {entry.type} · {entry.points_delta > 0 ? "+" : ""}
                {entry.points_delta}
              </p>
              <p className="text-mutedText">
                {entry.description || "Sin descripción"} · {new Date(entry.created_at).toLocaleString()}
              </p>
            </li>
          ))}
        </ul>
        ...
        <ul className="space-y-2 text-sm">
          {(loyaltyTx ?? []).map((tx) => (
            <li key={tx.id} className="rounded-md border border-uiBorder px-3 py-2">
              <p className="font-semibold">
                {tx.source_type} · {tx.status} · {tx.points_delta > 0 ? "+" : ""}
                {tx.points_delta}
              </p>
              <p className="text-mutedText">
                source_id: {tx.source_id} · {new Date(tx.created_at).toLocaleString()}
              </p>
            </li>
          ))}
        </ul>
