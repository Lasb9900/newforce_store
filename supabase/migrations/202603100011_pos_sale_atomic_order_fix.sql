-- Reorder POS transaction flow to: order -> items -> stock update (single atomic transaction).
-- Any error raises exception and rolls back the full statement.
...
begin
  ...
  set qty = v_stock - p_qty,
      ...
      updated_at = now()
  where id = p_product_id
    and coalesce(qty, base_stock) >= p_qty;

  get diagnostics v_stock_rows = row_count;
  if v_stock_rows = 0 then
    raise exception 'No se pudo actualizar stock de forma segura';
  end if;
