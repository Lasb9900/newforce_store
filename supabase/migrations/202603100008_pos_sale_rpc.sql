...
begin
  ...
  insert into orders (
    ...
    p_customer_id, p_sold_by, nullif(trim(coalesce(p_customer_email, '')), ''),
    'paid', 'paid', p_payment_method, v_ref,
    v_total, v_total, 'USD', 'physical_store', v_points
  )
...
grant execute on function public.create_pos_sale(uuid, int, text, text, text, uuid, uuid) to authenticated, service_role;
